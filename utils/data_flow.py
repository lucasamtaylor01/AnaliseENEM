...
def load_or_process_data(year: int) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if processing_files_exist(year):
        print(f"Processed data for {year} already exists. Skipping processing step...\n")
    ...
    df_participants_raw, df_results_raw = load_raw_data(year)

    print(f"Raw data for {year} loaded successfully.\n")
    ...
    (
        ...
        df_pre_clustering_state,
        x_scaled_state,
    ) = process_data(df_participants_raw, df_results_raw, year)
    print(f"Data for {year} processed successfully.\n")

    print(f"Saving processed city data for {year}...\n")
    ...
    print(f"Saving state model data for {year}...\n")
